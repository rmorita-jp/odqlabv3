function [x,exitflag,T,FOPT] = odqlp_sdpt3(G,T,gamma)
...
%%%%%initialize%%%%%
if gamma.wv==inf
    vectorf = sparse( 1 + m*m*T + p*m*(T-1) , 1 );
else
    vectorf = sparse( 1 + m*m*T*2 + p*m*(T-1) , 1 );
end
...
%%%%%%%%%%finalize%%%%%%%%%%
if gamma.wv==inf
    matrixA = sparse([ -ones( p , 1 )         zeros( p , m*m*T )  eye_sumE                 ;...
                zeros( p*m*(T-1) , 1 )   PHI              -eye( m*p*(T-1) )         ;...
                zeros( p*m*(T-1) , 1 )  -PHI              -eye( m*p*(T-1) )         ]);
else
    matrixA = sparse([ -ones( p , 1 )         zeros( p , m*m*T ) zeros( p , m*m*T )           eye_sumE                 ;...
...
%         matrixA*x <= vectorb;
% cvx_end
% blk{1,1}='l'; blk{1,2}=length(vectorf);
% At{1}=speye(length(vectorf));
% C{1}=sparse(length(vectorf),1);
blk{1,1}='u'; blk{1,2}=length(vectorf);
At{1}=matrixA;
C{1}=vectorf';
OPTIONS = sqlparameters;
[obj,X,y,Z,info,runhist] = sqlp(blk,At,C,vectorb,OPTIONS);
x=X;
exitflag=1;
FOPT='null';
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
